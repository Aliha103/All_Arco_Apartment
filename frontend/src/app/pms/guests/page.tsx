...
function BookingCard({ booking, onClick }: BookingCardProps) {
  ...
  return (
    <Card
      onClick={onClick}
      className="border-gray-200 shadow-sm hover:shadow-md transition-all cursor-pointer group hover:border-blue-300"
    >
      <CardContent className="p-4">
        {/* Header Row - Booking ID & Status */}
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base font-bold text-gray-900 truncate">{booking.booking_id}</h3>
          <Badge className={cn('text-[10px] font-medium px-2 py-0.5 border', statusConfig.color)}>
            {statusConfig.label}
          </Badge>
        </div>

        {/* Primary Guest - Compact */}
        <div className="mb-2.5">
          <p className="text-[10px] uppercase tracking-wide text-gray-500 font-medium mb-0.5">Primary Guest</p>
        ...
        <div className="flex items-center justify-between mb-2.5 text-xs">
          <div className="flex-1">
            <p className="text-[10px] uppercase tracking-wide text-gray-500 font-medium mb-0.5">Check-in</p>
            <p className="text-xs font-medium text-gray-900">{formatDate(booking.check_in_date)}</p>
          </div>
          <div className="flex-1">
            <p className="text-[10px] uppercase tracking-wide text-gray-500 font-medium mb-0.5">Check-out</p>
            <p className="text-xs font-medium text-gray-900">{formatDate(booking.check_out_date)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between pt-2.5 border-t border-gray-100">
          ...
          <span className="text-sm font-semibold text-gray-900">{booking.number_of_guests}</span>
        </div>
      </CardContent>
    </Card>
  );
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-start justify-between mb-6 pb-4 border-b border-gray-200">
          <div>
            ...
            {guest.relationship && (
              <p className="text-sm text-gray-600 mt-1">Relationship: {guest.relationship}</p>
            )}
          </div>
          {guest.is_primary && (
            <Badge className="bg-blue-600 text-white border-0 px-3 py-1">Primary Guest</Badge>
          )}
        </div>

        {/* Personal Information Section */}
        <div className="mb-6">
          <h5 className="text-sm font-semibold text-gray-900 mb-3 uppercase tracking-wide flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {guest.email && (
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="text-sm text-gray-900 font-medium">{guest.email}</p>
              </div>
            )}
            ...
            {guest.date_of_birth && (
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="text-sm text-gray-900 font-medium">{formatDate(guest.date_of_birth)}</p>
              </div>
            )}
            ...
            {guest.country_of_birth && (
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="text-sm text-gray-900 font-medium">{guest.country_of_birth}</p>
              </div>
            )}
            ...
            {guest.birth_city && (
              <div className="bg-gray-50 p-3 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  <MapPin className="w-3.5 h-3.5 text-gray-400" />
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide">Birth City/Province</p>
                </div>
        ...
        <div className="mb-6 bg-blue-50 p-4 rounded-lg">
          <h5 className="text-sm font-semibold text-gray-900 mb-4 uppercase tracking-wide flex items-center gap-2">
          ...
          {guest.document_number ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-white p-3 rounded-lg border border-blue-100">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Document Number</p>
                <p className="text-sm text-gray-900 font-mono font-semibold">{guest.document_number}</p>
              </div>

              {guest.document_issue_date && (
                <div className="bg-white p-3 rounded-lg border border-blue-100">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Issue Date</p>
                  <p className="text-sm text-gray-900 font-medium">{formatDate(guest.document_issue_date)}</p>
                </div>
              )}

              {guest.document_expire_date && (
                <div className="bg-white p-3 rounded-lg border border-blue-100">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Expiry Date</p>
                  <p className="text-sm text-gray-900 font-medium">{formatDate(guest.document_expire_date)}</p>
                </div>
              )}

              {guest.document_issue_country && (
                <div className="bg-white p-3 rounded-lg border border-blue-100">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Issue Country</p>
                  <p className="text-sm text-gray-900 font-medium">{guest.document_issue_country}</p>
                </div>
              )}

              {guest.document_issue_city && (
                <div className="bg-white p-3 rounded-lg border border-blue-100">
                  <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Issue City/Province</p>
                  <p className="text-sm text-gray-900 font-medium">
                    {guest.document_issue_city}
            ...
            <div className="text-center py-4">
              ...
              <p className="text-sm text-gray-500">No document information provided</p>
            </div>
          )}
        </div>
        ...
        {guest.note && (
          <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
            <h5 className="text-sm font-semibold text-gray-900 mb-2 uppercase tracking-wide">Internal Notes</h5>
            <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{guest.note}</p>
          </div>
        )}
      </CardContent>
    </Card>
