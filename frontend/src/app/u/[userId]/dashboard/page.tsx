...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && !isAuthorized) {
      ...
      router.replace(`/u/${correctUserId}/dashboard`);
    }
  }, [authLoading, user, isAuthorized, router]);

  // Team members should use PMS instead
  useEffect(() => {
    if (!authLoading && user && isAuthorized && isTeamMember) {
      router.replace('/pms');
    }
  }, [authLoading, user, isAuthorized, isTeamMember, router]);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-gray-600">
            {authLoading ? 'Loading...' : isTeamMember ? 'Redirecting to PMS...' : 'Verifying access...'}
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <Link href="/" className="text-2xl font-bold text-[#C4A572]">
            All'Arco Apartment
          </Link>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <Card>
            <CardContent className="pt-6">
              ...
              <p className="text-3xl font-bold text-[#C4A572]">{upcomingBookings.length}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
